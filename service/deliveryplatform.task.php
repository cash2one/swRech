...
namespace service
{
class deliveryplatform_task extends \service_class
{
...
private	static	function rsp_delivery($dbkey, $get_func, $send_ok_func, $send_fail_func, $next=1)
{
	...
	$rdata = array_intersect_key($rdata, $saveData);
	if($res)
	{//@成功
		$send_ok_func($order);
		$rdata['time'] = time();
...
{
	var_dump($data);
	if(!$cdata = \platform::data($data['appid']))
	{
		write_warn('game_server_err', 'unknown appid ['.$data['appid'].']');
		return false;
	}
...
//	$data['cash'] = 1;
	...
	{
		write_log('http_rech_fail',array($url,$port,$res));
		return false;
	}
	...
	if('{"ret":0}' !== $res)
	{
		write_warn('send_recharge', $url.' - fail gid:['.($data['appid']).'] and ['.$res.']');
		return false;
	}
