...
namespace service 
{
class log_task extends \service_class
{
//@定义内部变量存储一些数据
//@private	static	$args;
...
public	static	function setup()
{
	...
	$serv->after(1000*$serv->worker_id, function()use($serv){$serv->tick(LOG_SAVE_MIN_LIMIT_TIME,array(__CLASS__,'resp_time_save'));});
...
public	static   function  make_log( $name, $value )
{
	...
	{
		write_log('task_log_err', '未定义name:'.$name);
		return false;
	}
	...
	{
		write_log('task_log_err', json_encode(array($name, $value)));
		return false;
	}
...
private	static	function send_log($name, $cache)
{
	...
	$res = \service\mysql::insertLogDB($logDefine["table"], $logDefine["column"], $sendData);
