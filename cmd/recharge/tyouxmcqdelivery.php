...
* auther zzd
*/
class tyouxmcqdelivery extends \request_class
{
/*
* cmd and cmd arg`s count must modity
*/
const CMD_NUM	= '/tyou_xmcq_delivery.xl';
const ARG_COUNT	= 0;
...
public	static	function handler($request,$response)
{
	...
	if( $data['appid'] !== SECRET_TIANYOUXMCQ_APPID)
	{
	...
	$str	= \sign::make_string($data).'&'.SECRET_TIANYOUXMCQ_APPKEY;

	if($sign !== md5($str))
	{
		write_log('tyouxmcq_fail','sig'.$str);
		$response->end('{"code":3}');
		return;
	}
	...
	if(isset($data['custom_info']) )
	{
		...
		$origin	= SECRET_TIANYOUXMCQ_ORIGIN;
	}
	else
	{
		$response->end('{"code":404}');
	...
	{
		write_log('tyouxmcq_fail', 'set_deliveryed_status:'.$order);
