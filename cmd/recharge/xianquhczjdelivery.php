...
* auther zzd
*/
class xianquhczjdelivery extends \request_class
{
/*
* cmd and cmd arg`s count must modity
*/
const CMD_NUM	= '/xianqu_hczj_delivery.xl';
const ARG_COUNT	= 0;
...
public	static	function handler($request,$response)
{
	...
	if( $data['cpid'] !== SECRET_XIANQUHCZJ_CPID)
	{
...
//	}
	...
	$temp	= implode('&', $temp).'&SecretKey='.SECRET_XIANQUHCZJ_SECRET;

	if($sign !== md5($temp))
	{
		write_log('xianquhczj_fail','sig'.$temp);
		$response->end('fail');
		return;
	}
	...
	if(isset($data['ext_info']) )
	{
		...
		$order	= $data['ext_info'];
		$origin	= SECRET_XIANQUHCZJ_ORIGIN;
	}
	else
	{
		$response->end('{"code":404}');
	...
	{
		write_log('xianquhczj_fail', 'set_deliveryed_status:'.$order);
