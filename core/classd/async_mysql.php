	...
	$res = $mysqli->query($sql, function($mysqli, $r)
	{
		...
		if($r === true)
		{
			if(!is_null($db['callback']))
				call_user_func_array($db['callback'], array($r, $db['transfer'], $mysqli->affected_rows, $mysqli->insert_id));//#$mysqli->insert_id;
		}
		elseif(is_array($r))
		{
			if(!is_null($db['callback']))
		...
		else
		{
			...
			if(!is_null($db['callback']))
				call_user_func_array($db['callback'], array(false,$db['transfer'], $mysqli->errno));
...
{//@目前只支持LOG库
	...
	$serv->tick(3600000, array(__CLASS__, 'check_mysql'));
...
public	static	function resp_check($r)
{
	$id		= self::$serv->worker_id;

	if($r)
		write_log('mysql_check', 'suc:'.$id);
	else
		write_log('mysql_check', 'fal:'.$id);
}
